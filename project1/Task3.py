...
if __name__ == '__main__':
    ...
    for call in calls:
        if bangalore_call(call):
            prefix.append(extract_area_code(call))

    prefix_resum = list(set(prefix))
    prefix_resum.sort()

    print("\n The numbers called by people in Bangalore have codes:")
    for related_prefix in prefix_resum:
        print(related_prefix)

    # Part B
    recurrent_call = 0
    for call_prefix in prefix:
        if call_prefix == "(080)":
            recurrent_call += 1

    print("\n {} percent of calls from fixed lines in Bangalore are calls to other fixed lines in Bangalore.".format(
       round(recurrent_call/len(prefix)*100, 2)))
